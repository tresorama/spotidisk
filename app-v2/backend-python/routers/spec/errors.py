...
class HttpUnexpectedError_CodeShouldBeUnreachable(
  HttpError[
    Literal[500], 
    Literal["UNEXPECTED_ERROR_CODE_SHOULD_BE_UNREACHABLE"],
  ]
):
  """Unexpected error that we raise in HTTP handler when a cod branch that should be unreachable is reached"""
  httpStatus: Literal[500] = 500
  code: Literal["UNEXPECTED_ERROR_CODE_SHOULD_BE_UNREACHABLE"] = "UNEXPECTED_ERROR_CODE_SHOULD_BE_UNREACHABLE"
...
async def fastApiHttpExceptionHandlerOverwrite(request, exc):
  ...
  """
  # if is our custom error class (HttpError)...
  if isinstance(exc, StarletteHTTPException) and isinstance(exc.detail, HttpError):
